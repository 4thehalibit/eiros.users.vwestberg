{ pkgs, ... }:
{
  ...
  programs.zsh.shellAliases = {
    rebuild = "{ [ -d /tmp/eiros ] || gh repo clone lcleveland/eiros /tmp/eiros; cd /tmp/eiros && git pull; } && sudo nixos-rebuild switch --impure --flake '/tmp/eiros#default' --override-input eiros_users '/home/vwestberg/eiros-config' --override-input eiros_hardware 'github:4thehalibit/eiros.hardware.vwestberg' && [ -f ~/eiros-config/dms-settings.json ] && cp --remove-destination ~/eiros-config/dms-settings.json ~/.config/DankMaterialShell/settings.json || true";
